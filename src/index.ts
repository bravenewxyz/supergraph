...
const outPath =
  outArg !== -1
    ...
    : resolve(ROOT, "audit/supergraph.html");

const cfg = await loadConfig(ROOT);
const auditDir = resolve(ROOT, "audit/packages");
const packagesDir = cfg.workspace.packagesDir ?? "packages";
const EXT_ALIASES = cfg.supergraph.extAliases as [string, string][];
const PATH_SEGS = cfg.supergraph.pathSegments as [string, string][];

console.log("Building supergraph from audit data...");
const t0 = Date.now();
const data = await buildSupergraph(auditDir, packagesDir);
