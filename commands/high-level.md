...
2. **If MCP is not available** (tool not found or connection refused): Fall back to reading the static file. Find `.supergraph/context/architecture-compact.txt` in the current project. If that file does not exist, use the legacy compatibility output `.supergraph/supergraph-compact.txt`. If neither exists, run `supergraph --no-anim` to generate it. Read the entire file — every line, no truncation, no skipping. Read it in chunks if needed until you have consumed all of it. After reading, confirm: "Read architecture-compact.txt — [N] lines." and nothing else unless the user asks a follow-up question.
